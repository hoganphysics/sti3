...
class TestDevice(stidevicepy.LocalDevice):
    def __init__(self, name, address, module, targetServer):
        ...
        self.addChannel(2, stidevicepy.ChannelType.Input, stidevicepy.MixedValueType.Vector, stidevicepy.MixedValueType.Double, "test2")
        self.addChannel(3, stidevicepy.ChannelType.Input, stidevicepy.MixedValueType.Vector, stidevicepy.MixedValueType.Vector, "test3")

        engineID = stidevicepy.EngineID(0)
        self.addEventEngine(engineID)
        #self.addAttribute("test", "77")

        self.addAttribute("test", "77").setSetter(self.testSetter).setRefresher(self.testRefresher)
        return
    def __del__(self):
        print("&&&&&&&&& TestDevice del")
    def writeChannel(self, channel, value):
        print("write: " + str(channel) + " : " + str(value))
        return True

    def testSetter(self, value):
        print('inside custom setter')
        self.testVal = int(value)
        if int(value) < 21:
            return True
        else:
            return False
    def testRefresher(self):
        return str(2*self.testVal)
...
#            print("++++++ collect 3 -> 1")
            for m in tmpMeas:
                val=stidevicepy.MixedValue([3,2,1])
#                val=stidevicepy.MixedValue([123, 8, "test",[43,"abc"], 99.4])
#                print("++++++ collect 3 -> 2")
                #val2=stidevicepy.MixedValue([43,"abc"])
#                val.addValue([43,"abc"])
...
dev1=TestDevice("TestDevice2", "localhost", 1, "localhost/0/BridgeServer")
