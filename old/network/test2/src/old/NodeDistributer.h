#ifndef STI_UTILS_NODEDISTRIBUTER_H
#define STI_UTILS_NODEDISTRIBUTER_H
...
public:
	...
	NodeDistributer(const CollectionT_ptr& nodeCollector)
	{
		nodes = nodeCollector;
	}
	...
	bool add(const ID& id, const T_ptr& node)
	{
		...
		NodeTCollection_ptr collNode = std::static_pointer_cast<Node<ID, T>>(node);
